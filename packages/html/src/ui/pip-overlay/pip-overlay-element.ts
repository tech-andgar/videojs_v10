...
export class PipOverlayElement extends MediaElement {
  ...
  protected override willUpdate(changed: PropertyValues): void {
    ...
    if (changed.has('pipSrc')) {
      log('pipSrc changed:', this.pipSrc);
  ...
  protected override update(changed: PropertyValues): void {
    ...
    if (state.pipOverlayActive) {
      log(
        'show → src=%s pos=(%s,%s) scale=%s',
        state.pipOverlaySrc,
        state.pipOverlayPosition.x,
        state.pipOverlayPosition.y,
        state.pipOverlayScale
      );
      this.dataset.active = '';
    } else {
      log('hide');
  ...
  readonly #onPointerDown = (e: PointerEvent) => {
    ...
    const onPointerUp = (upEvt: PointerEvent) => {
      ...
      log(
        isResize ? 'resize end scale=%s' : 'drag end pos=(%s,%s)',
        isResize ? state.pipOverlayScale : state.pipOverlayPosition.x,
        isResize ? '' : state.pipOverlayPosition.y
      );
  ...
  readonly #onError = () => {
    ...
    if (__DEV__ && code === MediaError.MEDIA_ERR_DECODE) {
      console.warn(
        '[pip-overlay] MEDIA_ERR_DECODE on PIP video. On iOS Safari this may indicate the hardware video decoder limit has been reached. Consider reducing concurrent video elements.'
      );
    }
